...
import { getDatabase, ref as dbRef, set, onChildAdded, push, onChildChanged } from "https://www.gstatic.com/firebasejs/9.21.0/firebase-database.js";
...
window.addEventListener('load', function () {
    ...
    const postsRef = dbRef(database, 'Web Chat');
...
document.getElementById("send-button").addEventListener('click', async function (e) {
    ...
    if (file && !img) {
        ...
        if (username && email) {
            const newPostRef = push(dbRef(database, 'Web Chat/'));
