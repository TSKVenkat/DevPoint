...
import { getDatabase, ref as dbRef, set, onChildAdded, push} from "https://www.gstatic.com/firebasejs/9.21.0/firebase-database.js";
...
window.addEventListener('load', function () {
    ...
    const postsRef = dbRef(database, 'Canvas Chat');
...
document.getElementById("send-button").addEventListener('click', async function (e) {
    ...
    if (file && !img) {
        ...
        if (username && email) {
            const newPostRef = push(dbRef(database, 'Canvas Chat/'));
